// C++ FFI bot for fantastic_bits. Build + run via cargo (from workspace root):
//
...
//
// Strategy mirrors `v1/fantastic_bits.cpp` (the paste-ready snapshot
// for the CodinGame web editor) function-for-function — the only
// differences are I/O wrapping. v1 reads stdin / writes stdout; this
// file consumes `TurnInputFFI` and returns a `TurnResult<TurnOutput>`.
//
// If you tune the strategy here, mirror the change into v1 so the
// snapshot stays current with what we'd actually submit.

#include "../../defs/include/fantastic_bits_defs.h"
...
static int g_petr = 0;  // two-tick Petrificus guard, preserved across turns.
...
LocalEntity from_ffi(const Entity& e) {
    LocalEntity le;
    le.id = e.id;
    le.x = e.x;
    le.y = e.y;
...
int square_dist(const LocalEntity& a, const LocalEntity& b) {
    return (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y);
}

int square_dist(const LocalEntity& a, int x, int y) {
    return (a.x - x) * (a.x - x) + (a.y - y) * (a.y - y);
}

std::pair<int, int> find_target(const LocalEntity& source, int dest_x, int dest_y) {
    return {static_cast<int>(dest_x - source.vx),
            static_cast<int>(dest_y - source.vy)};
}

int index_of_snaffle(const std::vector<LocalEntity>& snaffles, int id) {
...
WizardAction make_move(int x, int y, int power) {
    return WizardAction{ActionKind::Move, x, y, power, 0};
}

WizardAction make_throw(int x, int y, int power) {
    return WizardAction{ActionKind::Throw, x, y, power, 0};
}

WizardAction make_spell(ActionKind kind, int target_id) {
...
TurnOutput decide(std::vector<LocalEntity>& wizards,
...
TurnResult<TurnOutput> take_turn(TurnInputFFI input) {
    ...
    for (std::uintptr_t i = 0; i < input.num_entities; ++i) {
        LocalEntity le = from_ffi(input.entities[i]);
        switch (le.type) {
            case 0:
                wizards.push_back(le);
                break;
            case 1:
                opponents.push_back(le);
                break;
            case 2:
                snaffles.push_back(le);
                break;
            case 3:
                bludgers.push_back(le);
                break;
        }
    }

    TurnOutput output = decide(wizards, opponents, snaffles, bludgers, input.my_magic);
