...
// ---- InitialInput (empty for tic-tac-toe — uses `NoInitialInput`) ----
//
...
//                                 stays game-agnostic.
// Tic-tac-toe doesn't ship per-player init data, so the structs are
// empty and `operator>>` is a no-op. When you grow a real
// `InitialInput`, mirror fantastic_bits's `_defs_io.h` and update the
// typedef.
