// C++ FFI bot for tron (v1). See ../baseline_cpp/bot.cpp for the
// transport contract; the only thing that changes per bot is the
// strategy namespace included from `strategy.h`.
...
void initialize(cgio::InitialInputFfi input) {
    tron_v1_cpp::on_init(cgio::as_ref(input));
}
...
TurnResult<TurnOutput> take_turn(TurnInputFFI input) {
    ...
    TurnOutput output = tron_v1_cpp::decide(cgio::as_ref(input));
