// C++ stdio bot for spider_attack (baseline variant).
//
...
//     `cargo xtask bundle spider_attack baseline --lang cpp` runs cpp_flatten
//     on this file, inlining `strategy.h` + the transitive `_defs_io.h`
...
#endif
{
    ...
    spider_attack_baseline_cpp::on_init(init);

    TurnInput input;
    while (std::cin >> input) {
        std::cout << spider_attack_baseline_cpp::decide(input) << std::endl;
    }
    return 0;
}
